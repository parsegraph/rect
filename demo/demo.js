...
app.get("/", async (req, res) => {
  ...
  write(`<html>`);
  write(`<head>`);
  write(`<title>rect</title>`);
  write(`</head>`);
  write(`<body>`);
  write(
    `<h1>rect <a href='/coverage'>Coverage</a> <a href='/docs'>Docs</a></h1>`
  );
  write(
    `<p>This library is available as JavaScript UMD module: <a href='/parsegraph-rect.js'>parsegraph-rect.js</a></p>`
  );
...
app.listen(port, () => {
  console.log(`See rect build information at http://localhost:${port}`);
});
